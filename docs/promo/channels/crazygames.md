...
| 视频 | CrazyGames 提交**不需要**视频,不用录(录了留给别的渠道用) |
...
官方规则:**必须 3 张**——16:9(1920×1080)、2:3(800×1200)、1:1(800×800);图上**只能出现游戏名**(不准写 "New / Play now" 等);不要边框、不要 logo、不要模糊;**不要直接用原始截图**,要有海报感;三张视觉风格必须一致。

做法(用已有的 `promo/assets/final/01-menu.png … 06-briefing.png` 当底):

1. 选一张最有戏剧性的截图(战列舰主炮齐射/风暴天最佳)
2. 调色(压暗四周、提高对比,做出海报感,避免"原始截图"观感)
3. 叠上游戏名 `IRON TIDE`(只写这三个词,粗衬线或军事风字体,别加任何其它文字)
4. 同一张底图裁出三个比例,保证三张一眼认得出是同一款游戏

输出文件名:

```
promo/assets/final/og-1280x720.png
promo/assets/final/03-broadside.png（需竖裁）
promo/assets/final/thumb-ph-240x240.png
```
...
| Build | 上传 `promo/builds/irontide-portal-singleplayer.zip` |
| Covers | 上传 §1.6 的三张图 |
| Video | 不需要 |
...
```
Iron Tide contains stylized military combat (ships, planes, tanks). There is no blood or gore, no profanity, no sexual content, no gambling, and no drug references. Destroyed vehicles sink or are removed; no depictions of human injury. Comparable to mild-violence ratings such as PEGI 7-12.
```
...
- **Full Launch 要求**:SDK 必接(初始化 + gameplay start/stop 事件 + 广告位事件);内容不超过 PEGI-12(Iron Tide 无血腥,符合);初始下载 ≤50MB(1.4MB,符合);快速进入 gameplay(菜单即达,符合)
